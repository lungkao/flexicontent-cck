...
var ffieldid='columnchoose_'+data_tbl_id+'_'+col;var ffieldname='columnchoose_'+data_tbl_id+'['+col+']';str=str+'<li><input id="'+ffieldid+'" name="'+ffieldname+'" type="checkbox" data-colno="'+col+'" '+checked_str+' onclick="fc_tbl_toggle_column(\''+container_div_id+'\', \''+data_tbl_id+'\', '+col+', 0);">'
+'<label id="'+ffieldid+'_label" style="color:'+fontcolor_str+';" for="'+ffieldid+'">'+col_display_name+'</label></li>';}
str='<span id="'+container_div_id+'_cols"><ul>'+str+'</ul></span>'+end_text;document.getElementById(container_div_id).innerHTML=str;column_toggles_update_data(container_div_id,data_tbl_id);}
...
{sel=typeof sel!=='undefined'&&sel?sel:'body';var sbox=jQuery(sel);var _s2ver=(jQuery.fn.select2&&jQuery.fn.select2.amd)?4:3;s2_elems=s2_elems||sbox.find('select.use_select2_lib');if(window.skip_select2_js)
...
var sel_EL=jQuery(this);var sel2ops={minimumResultsForSearch:10};if(_s2ver>=4){sel2ops.templateResult=function(item){if(!item.element)return item.text;var dt=jQuery(item.element).attr('data-title');return dt?jQuery('<span title="'+dt+'">'+item.text+'</span>'):item.text;};}else{sel2ops.formatResult=fc_formatSel2Option;}
...
{var initData=sel_EL.data('select2-initdata');if(_s2ver>=4){var ids=Array.isArray(initData)?initData.map(function(d){return d.id||d;}):(initData?initData.id||initData:null);if(ids)sel_EL.val(ids).trigger('change');}else{sel_EL.select2('data',initData);}}
...
{sel_EL.data('field_being_edited',null);sel_EL.trigger('change');}}).on('select2-selecting select2:selecting',function(e)
...
{jQuery('#select2-drop').find('.select2-selected-visible, .select2-selected').removeClass('select2-selected-visible').removeClass('select2-selected');jQuery('#select2-drop').find('.select2-selected').removeClass('select2-selected-visible').removeClass('select2-selected');if(_s2ver>=4){sel_EL.data('set_selected_values',null).val(nVals).trigger('change');}else{sel_EL.data('set_selected_values',null).select2('val',nVals).trigger('change');}}
if(!sel_EL.attr('multiple')&&sel_EL.hasClass('fc_add_highlight'))
